...
VOID KdInitSerial(VOID) {
	IoOutputByte(COM1 + COM_INTERRUPT_ENABLE_REG,
				 0x00); // Disable all interrupts.

	IoOutputByte(COM1 + COM_LINE_CONTROL_REG,
				 0x80); // Enable DLAB. This is to set the baud divisor.
	IoOutputByte(COM1 + COM_DATA_REG,
				 0x03); // Set divisor to 3. 115200 / 3 = 38400 baud.
	...
	IoOutputByte(COM1 + COM_MODEM_CONTROL_REG,
				 0x1E); // Enable loopback mode. We are going to test the UART.
	IoOutputByte(COM1 + COM_DATA_REG,
				 0xAE); // Send 0xAE and test if the UART returns the same byte.
