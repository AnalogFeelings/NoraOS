...
struct limine_framebuffer_legacy_response {
	uint64_t revision;
	uint64_t framebuffer_count;
	LIMINE_PTR(struct limine_framebuffer_legacy **)
	framebuffers;
};
...
struct limine_framebuffer_legacy_request {
	uint64_t id[4];
	uint64_t revision;
	LIMINE_PTR(struct limine_framebuffer_legacy_response *)
	response;
};
...
struct limine_terminal_legacy_response {
	uint64_t revision;
	uint64_t terminal_count;
	LIMINE_PTR(struct limine_terminal_legacy **)
	terminals;
	LIMINE_PTR(limine_terminal_legacy_write)
	write;
};
...
struct limine_terminal_legacy_request {
	uint64_t id[4];
	uint64_t revision;
	LIMINE_PTR(struct limine_terminal_legacy_response *)
	response;
	LIMINE_PTR(limine_terminal_legacy_callback)
	callback;
};
